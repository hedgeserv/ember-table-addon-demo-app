Feature: Lazily Loaded Set of Loans
  ...
  Scenario: Show first and last section of loans
    ...
    Then Only first chunk was loaded in total 300 in first time

  @complete
  Scenario: Scroll bar to load other sections of loans
    Given There are 200 loans in chunk size 50
    Then Scroll bar by offset 60 with 3 times to load next chunks in total 200 and drag scroll bar to top without rerender
